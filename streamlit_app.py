...
st.caption("Incolla un link Google Drive (XLSX) **oppure** carica il file .xlsx. Poi premi **Calcola KPI**.")

# ---------------------- Helpers ----------------------
PAIR_DEFAULT = "SOL/USDC"

def extract_drive_id(url: str) -> str | None:
    """Estrae il fileId da vari formati di URL Google Drive."""
    if not url:
        return None
    patterns = [
...
def _find_col(df: pd.DataFrame, candidates: List[str]) -> str:
    """Trova il nome colonna effettivo tra i candidati (case/space insensitive)."""
    clean = {re.sub(r"\s+", "", c).lower(): c for c in df.columns}
    for cand in candidates:
        key = re.sub(r"\s+", "", cand).lower()
        if key in clean: return clean[key]
    # fallback: cerca per include
    for c in df.columns:
...
def sanitize_orders(df_raw: pd.DataFrame) -> pd.DataFrame:
    """
    Pulisce il foglio 'sheet1' in stile Binance esportato, eliminando righe-etichette duplicate
    e tenendo solo BUY/SELL con colonne standardizzate.
    """
    # Teniamo solo righe con Type BUY/SELL
...
def fifo_trades(orders: pd.DataFrame, pair: str) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """
    Costruisce coppie BUY->SELL in FIFO, consentendo split di ordini.
    Ritorna (trades, meta) dove trades contiene lista di trade chiusi.
    """
...
with st.sidebar:
    st.header("⚙️ Input")
    drive_url = st.text_input("Link Google Drive del file XLSX", placeholder="https://drive.google.com/file/d/…/view")
...
if go:
    ...
    try:
        if uploaded is not None:
            ...
            if isinstance(x, bytes):
                ...
                # gdown può tornare path: leggi il file
                with open(x, "rb") as f:
                    xlsx_bytes = f.read()
        else:
    ...
    st.info("⬅️ Inserisci link o carica il file e premi **Calcola KPI**. La dashboard calcola i KPI con pairing FIFO BUY→SELL e include il residuo configurabile (default 15 SOL @ 201 → 220).")
